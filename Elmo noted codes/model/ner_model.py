...
class NERModel(nn.Module):
    ...
    def __init__(self, config):
        ...
        if not self.use_elmo: # 如果不使用elmo
            self.emb = nn.Embedding(self.config.nwords, self.config.dim_word, padding_idx=0) # word embedding
            self.char_embeddings = nn.Embedding(self.config.nchars, self.config.dim_char, padding_idx=0) # char embedding
            self.char_lstm = nn.LSTM(self.config.dim_char, self.config.hidden_size_char, bidirectional=True) # char BiLSTM

        self.dropout = nn.Dropout(p=self.config.dropout)
        
        # word BiLSTM
        self.word_lstm = nn.LSTM(self.config.dim_elmo if self.use_elmo \ # 如果使用elmo，那么输入的维度是config.dim_elmo
                                 else self.config.dim_word+2*self.config.hidden_size_char, # 如果不使用elmo，那么输入维度是word_embedding + 2 * char_lstm的输出
    ...
    def forward(self, input):
        ...
        if self.use_elmo: # 如果使用elmo
            word_emb = self.dropout(input.transpose(0,1)) # 直接接 dropout

        else: # 如果不用elmo，则自己训练词向量
...
class LinearBlock(nn.Module):
    def __init__(self, ni, nf, drop):
        super().__init__()
        self.lin = nn.Linear(ni, nf) # linear (lstm hidden size * 2, ntags)
        self.drop = nn.Dropout(drop) # dropout
        self.bn = nn.BatchNorm1d(ni) # BN
