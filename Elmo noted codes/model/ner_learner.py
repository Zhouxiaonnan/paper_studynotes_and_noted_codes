...
class NERLearner(object):
    ...
    Contains methods for training and testing the model
    """
    def __init__(self, config, model):
        super().__init__()
        self.config = config # 参数
        self.logger = self.config.logger
        self.model = model # 模型（Nermodel）
        ...
        if self.use_elmo: # 如果使用elmo
            ...
            # 导入elmo，需要下载allennlp包，并导入allennlp.modules.dlmo.Elmo
            # 参数中的2表示取elmo lstm层中的几层
    ...
    def batch_iter(self, train, batch_size, return_lengths=False, shuffle=False, sorter=False):
        ...
        """
        nbatches = (len(train) + batch_size - 1) // batch_size # batch的个数

        def data_generator():
            while True:
                if shuffle: train.shuffle() # 是否打乱
                elif sorter==True and train.sorter: train.sort() # 不打乱则进行排序

                for i, (words, labels) in enumerate(minibatches(train, batch_size)): # 转换成minibatch
                    ...
                    # 如果使用elmo，则word_ids使用的是words的原始格式，比如[i go to school]
                    if self.use_elmo:
                        word_ids = words

                    if labels:
                        labels, _ = pad_sequences(labels, 0) # Labels 的 pad
                        # if categorical
    ...
    def fit(self, train, dev=None, epochs=None, fine_tune=False):
        ...
        """
        if not epochs:
            epochs = self.config.nepochs # 循环次数
        batch_size = self.config.batch_size # batch_size
        
        # nbatches_train：训练集的batch数
        # train_generator：训练集batch内数据的生成器
        nbatches_train, train_generator = self.batch_iter(train, batch_size, # 输出batch
                                                          return_lengths=True)
        if dev: # 验证集
            nbatches_dev, dev_generator = self.batch_iter(dev, batch_size,
                                                      return_lengths=True)

        scheduler = StepLR(self.optimizer, step_size=1, gamma=self.config.lr_decay) # 优化器
    ...
    def train(self, epoch, nbatches_train, train_generator, fine_tune=False):
        ...
        prog = Progbar(target=nbatches_train) # 打印进度条的类
        
        # batch_idx：第几个 batch
        # inputs：如果使用elmo，就是原始格式[i, go, to, school]
        # targets：words的标签
        # sequence_lengths：如果return_lengths，就返回序列的长度
        for batch_idx, (inputs, targets, sequence_lengths) in enumerate(train_generator):
            
            # 如果到达最大batch次数，则break
            if batch_idx == nbatches_train: break
            if inputs['word_ids'].shape[0] == 1: # 如果batch里的单词只有一个，则不训练该batch，跳过
            ...
            if self.use_elmo: # 使用elmo的话，inputs['word_ids']是原始数据
                sentences = inputs['word_ids']
                character_ids = batch_to_ids(sentences) # 使用了elmo中将单词转换成id的方法，不使用外部的word2id
                if self.use_cuda:
                    character_ids = character_ids.cuda()
                embeddings = self.elmo(character_ids) # 使用 elmo 词向量 embedding
                word_input = embeddings['elmo_representations'][0] # 获取elmo第一层的embedding
                word_input, targets = Variable(word_input, requires_grad=False), \ # word_embedding不需要优化
                                      Variable(targets)
                inputs = (word_input)
                ...
                word_input, char_input, targets = Variable(word_input, requires_grad=False),                                                   Variable(char_input, requires_grad=False),                                                  Variable(targets)
    ...
    def test(self, nbatches_val, val_generator, fine_tune=False):
        ...
        for batch_idx, (inputs, targets, sequence_lengths) in enumerate(val_generator):
            ...
            if self.use_elmo:
                ...
                word_input, targets = Variable(word_input, requires_grad=False),                                       Variable(targets)
                inputs = (word_input)
    ...
    def predict_batch(self, words):
        ...
        if self.use_elmo:
            ...
            word_input, char_input = Variable(word_input, requires_grad=False),                                      Variable(char_input, requires_grad=False)
