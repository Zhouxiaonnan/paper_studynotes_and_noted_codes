...
# 这里让reduce=False，返回的是每一个单词的loss，而不是整个句子的loss
# 这样我们可以只让模型训练非pad部分，因为后面的pad都为0，没必要进行训练
...
def get_dev_loss(data_iter):
    ...
    for source_data, target_data_input, target_data in process_bar:
        ...
        weights = target_data.view(-1) != 0 # 根据词id对pad部分置零，非pad部分为1
        
        # loss * weights就可以直接得到非pad部分的loss
        # 平均也需要除以非pad部分的长度
...
def get_test_bleu(data_iter):
    ...
    for source_data, target_data_input, target_data in process_bar:
        ...
        out = model(source_data, target_input,mode="test") # 翻译
        out = np.array(out).T
        tmp_preds = []
        for i in range(out.shape[0]):
            tmp_preds.append([])
        for i in range(out.shape[0]):  # 对于翻译中的每个样本
            for j in range(out.shape[1]): # 对于每个样本的每个词
                if out[i][j]!=3: # 如果没有到达最后，因为3代表<EOS>
                    tmp_preds[i].append(out[i][j]) # append
                else: # 如果到达最后则break
                    break
        preds += tmp_preds # 所有预测结果
        tmp_refs = [] # 参考结果
        ...
        refs+=tmp_refs
    bleu = corpus_bleu(refs,preds)*100 # 将预测翻译结果和参考结果输入corpus_bleu，计算bleu值
